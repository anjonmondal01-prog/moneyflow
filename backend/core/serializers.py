...
class PersonSerializer(serializers.ModelSerializer):
 ...
 def get_outstanding_receivable(self,o):
  ...
  lent=sum((x.amount for x in o.transactions.filter(status='active',type='lend')),start=0)+sum((x.amount for x in o.transactions.filter(status='active',type='help',expected_return=True)),start=0); rep=sum((x.amount for x in o.transactions.filter(status='active',type='repayment_received')),start=0); return lent-rep
 def get_outstanding_payable(self,o):
  borrowed=sum((x.amount for x in o.transactions.filter(status='active',type='borrow')),start=0); rep=sum((x.amount for x in o.transactions.filter(status='active',type='repayment_made')),start=0); return borrowed-rep
...
class TransactionSerializer(serializers.ModelSerializer):
 ...
 def validate(self,data): FinancialEngine.validate_mutation(data,self.context['request'].user,self.instance); return data
